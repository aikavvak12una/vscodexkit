...
const MARKERS = {
  ...
  notifyV10: "codexpatch:v13:diagnostic-live-states-turn-output-aware-retry",
  ...
  webviewAutoRetry: "codexpatch:v4:webview-auto-retry-send-shared-lock",
...
const V10_NOTIFICATION_HANDLER = `e.push((()=>{/* codexpatch:v13:diagnostic-live-states-turn-output-aware-retry */
...
const APP_MAIN_AUTO_RETRY_PATCH =
  'case`codexpatch-auto-retry`:{/* codexpatch:v4:webview-auto-retry-send-shared-lock */try{let t=e.conversationId??e.threadId,n=t?(e.turnId||r.get(Nr,t)?.turnId):e.turnId,o=t?(e.model??r.get(Nr,t)?.params?.model??null):e.model;if(!t||!n){B.dispatchMessage(`codexpatch-diagnostic`,{event:`auto-retry-send-skip-missing-context`,conversationId:t||``,turnId:n||``});break bb35}let s=globalThis.__codexpatchRetrySent||(globalThis.__codexpatchRetrySent=new Map),c=t+`|`+n,l=Date.now(),u=s.get(c);if(u&&l-u<3e4){B.dispatchMessage(`codexpatch-diagnostic`,{event:`auto-retry-send-skip-duplicate`,conversationId:t,turnId:n});break bb35}for(let[e,t]of s)try{l-t>12e4&&s.delete(e)}catch{}s.set(c,l);try{let r=`codexpatch:auto-retry:`+c,i=localStorage.getItem(r),a=i?Number(i.split(`:`)[0]):0;if(a&&l-a<3e4){B.dispatchMessage(`codexpatch-diagnostic`,{event:`auto-retry-send-skip-shared-duplicate`,conversationId:t,turnId:n});break bb35}let o=l+`:`+Math.random().toString(36).slice(2);localStorage.setItem(r,o);await new Promise(e=>setTimeout(e,30));if(localStorage.getItem(r)!==o){B.dispatchMessage(`codexpatch-diagnostic`,{event:`auto-retry-send-skip-shared-lost`,conversationId:t,turnId:n});break bb35}}catch{}B.dispatchMessage(`codexpatch-diagnostic`,{event:`auto-retry-send`,conversationId:t,turnId:n,model:o||``});await W(`codexpatch-retry-turn-for-host`,{hostId:e.hostId??zr,conversationId:t,turnId:n,model:o});B.dispatchMessage(`codexpatch-diagnostic`,{event:`auto-retry-send-ok`,conversationId:t,turnId:n})}catch(t){B.dispatchMessage(`codexpatch-diagnostic`,{event:`auto-retry-send-error`,conversationId:e.conversationId||e.threadId||``,message:String(t).slice(0,500)})}break bb35}case`ipc-broadcast`:e.method===`automation-capability-event`&&e.sourceClientId===`desktop`&&e.version===ze(`automation-capability-event`)&&QO(r,i.getForHostId(e.params.hostId),e.params),uk({claimAppConnectOAuthCallback:p,isCompactWindow:d,message:e,navigate:a,queryClient:c});break bb35;case`thread-follower-start-turn-request`:';
